...
| 名称 | 类型 | 默认值 | 说明 | 版本 |
| --- | --- | --- | --- | --- |
| src-list | `string[]` | `undefined` | 图片列表 |  |
| show | `boolean` | `undefined` | 是否展示大图预览 |  |
| default-show | `boolean` | `undefined` | 默认展示大图预览 |  |
| current | `number` | `undefined` | 当前展示的图片的下标 |  |
| default-current | `number` | `0` | 默认展示的图片的下标 |  |
| render-toolbar | `(props: { nodes: { prev: VNode, next: VNode, rotateCounterclockwise: VNode, rotateClockwise: VNode, resizeToOriginalSize: VNode, zoomOut: VNode, zoomIn: VNode, download: VNode, close: VNode } }) => VNodeChild` | `undefined` | 工具栏的渲染函数 | `2.38.2` |
| show-toolbar | `boolean` | `true` | 图片放大后是否展示底部工具栏 |  |
| show-toolbar-tooltip | `boolean` | `false` | 是否展示工具栏的提示 | 2.24.0 |
...
| show | `boolean` | `undefined` | 是否展示大图预览 |  |
| default-show | `boolean` | `undefined` | 默认展示大图预览 |  |
| render-toolbar | `(props: { nodes: { prev: VNode, next: VNode, rotateCounterclockwise: VNode, rotateClockwise: VNode, resizeToOriginalSize: VNode, zoomOut: VNode, zoomIn: VNode, download: VNode, close: VNode } }) => VNodeChild` | `undefined` | 工具栏的渲染函数 |  |
| show-toolbar | `boolean` | `true` | 图片放大后是否展示底部工具栏 |  |
| show-toolbar-tooltip | `boolean` | `false` | 是否展示工具栏的提示 |  |
