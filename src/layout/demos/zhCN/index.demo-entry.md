...
| has-aside | `boolean` | `false` | 组件内部是否有边栏，如果有的话必须设为 `true` |
...
| --- | --- | --- | --- |
| bordered | `boolean` | `false` |  |
| collapse-mode | `'transform' \| 'width'` | `'transform'` | 如果设定为 `'width'`，Aside 的内容宽度将会被实际改变；如果设定为 `'transform'`，边栏将只会移动它的位置而不会改变宽度 |
| collapsed | `boolean` | `undefined` | 边栏是否折叠。只在 `position` 为 `'static'` 时生效 |
| collapsed-width | `number` | `48` |  |
| content-style | `string \| Object` | `undefined` | 可滚动内容节点的样式 |
| default-collapsed | `boolean` | `false` |  |
| inverted | `boolean` | `false` | 使用反转背景色 |
| native-scrollbar | `boolean` | `true` | 是否在自身使用原生滚动条。如果设定为 `false`, Aside 将会对内容使用 naive-ui 风格的滚动条 |
...
| show-collapsed-content | `boolean` | `true` | 是否在 aside 折叠后展示内部内容 |
...
出于性能和未来 SSR 相关的考虑，在 v2.3.0 后，对于包含 `n-layout-aside` 的 `n-layout` 组件，需要显示的设定 `has-aside`. 同时折叠对于 `position="absolute"` 的 `n-layout-aside` 不再生效。
