...
| justify-content | `'space-between' \| 'space-around' \| 'space-evenly'` | `undefined` | `flex` 布局 |
| size | `'small' \| 'medium' \| 'large'` | `'medium'` | 标签页的大小 |
...
| value | `string \| number` | `undefined` | 受控模式下的默认值 |
| on-add | `() => void` | `undefined` | 添加标签的回调函数 |
| on-close | `(name: string \| number) => void` | `undefined` | 删除标签的回调函数 |
...
| tab | `string \| VNode \| () => VNodeChild` | `undefined` | 标签显示的名称 |
| name | `string \| number` | `undefined` | 标签的名称 |
