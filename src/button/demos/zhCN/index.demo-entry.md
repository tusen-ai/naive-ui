...
| dashed | `boolean` | `false` | 按钮边框是否为虚线（不可与 `secondary`、`tertiary`、`quaternary` 同时使用） |  |
| disabled | `boolean` | `false` | 按钮是否禁用 |  |
| focusable | `boolean` | `true` | 按钮是否可以被聚焦 |  |
| ghost | `boolean` | `false` | 按钮是否透明（不可与 `secondary`、`tertiary`、`quaternary` 同时使用） |  |
...
| quaternary | `boolean` | `false` | 是否是四级按钮（不可与 `dashed`、`ghost`、`text` 同时使用） |  |
| render-icon | `() => VNodeChild` | `undefined` | 按钮图标的渲染函数 | 2.34.0 |
| round | `boolean` | `false` | 按钮是否显示圆角 |  |
| size | `'tiny' \| 'small' \| 'medium' \| 'large'` | `'medium'` | 按钮的尺寸 |  |
| secondary | `boolean` | `false` | 是否是次要按钮（不可与 `dashed`、`ghost`、`text` 同时使用） |  |
| strong | `boolean` | `false` | 按钮文字是否加粗 |  |
| tertiary | `boolean` | `false` | 是否是三级按钮（不可与 `dashed`、`ghost`、`text` 同时使用） |  |
| text | `boolean` | `false` | 是否显示为文本按钮（不可与 `secondary`、`tertiary`、`quaternary` 同时使用） |  |
