...
| label-field | `string` | `'label'` | The field name of option labels in `options`. | NEXT_VERSION |
| name | `string` | `undefined` | The name attribute of the radio elements inside the group. |  |
| options | `Array<{ label?: string, value: string \| number \| boolean, disabled?: boolean }>` | `undefined` | Options for the radio group. When set, the default slot is ignored. | NEXT_VERSION |
| size | `'small' \| 'medium' \| 'large'` | `'medium'` | Size. |  |
| value | `string \| number \| boolean \| null` | `null` | Checked value. |  |
| value-field | `string` | `'value'` | The field name of option values in `options`. | NEXT_VERSION |
